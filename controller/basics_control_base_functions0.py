...
zeta = 0.5
s = control.tf('s')
P_b = omega ** 2 * (1 / b * s + 1) / (s ** 2 + 2 * zeta * omega * s + omega ** 2)  # short way
# P_b = control.tf([omega ** 2 / b, omega ** 2], [1, 2 * zeta * omega, omega ** 2])  # long way
...
char_poly_tf = control.tf(P_d.den, 1) * control.tf(controller.den, 1) + control.tf(P_d.num, 1) * control.tf(controller.num, 1)
...
mag_a, phase_a, w_a = control.bode(P_a, w_shared, dB=True, deg=True)  # plot=False
...
print(f'Gain margin is', gm_dB,'(dB) at phase crossover frequency', wpc,'(deg)')
print(f'Phase margin is', pm,'(deg) at gain crossover frequency', wgc,'(deg)\n')
...
# %% Plot gang of 4
fig, ax = plt.subplots()
control.gangof4_plot(P_d, controller, omega=w_shared)  # [[S, PS], [CS, T]]
